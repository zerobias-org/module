...
description: Workflow execution protocols and agent coordination. Use when orchestrating multi-step workflows or coordinating between multiple agents.
---
...
edit test/unit/some-producer.test.ts  # Unit tests
edit test/e2e/some-producer.test.ts   # e2e tests (describeModule<T> from @zerobias-org/module-test-client)
→ VALIDATE: Tests complete for operation

# 6. RUN TESTS (MANDATORY!)
zbb test --slot local       # unit + e2e direct
zbb testDirect --slot local # e2e direct against real API
...
**Fix**: Run `zbb generate` first, use generated types
...
// 5. Write tests (unit in test/unit/, e2e in test/e2e/ via describeModule<T>)
// 6. Run tests - zbb test --slot local / zbb testDirect --slot local
// 7. Build - zbb build
```
