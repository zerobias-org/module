...
description: README.md template and structure standards. Use when creating or updating module README files.
---
...
\`\`\`bash
npm install @zerobias-org/connector-[vendor]-[service]
\`\`\`
...
\`\`\`typescript
import { newServiceName } from '@zerobias-org/module-vendor-service';
...
\`\`\`typescript
import { ConnectionProfile } from '@zerobias-org/module-vendor-service';
...
Run from the module directory (`package/<vendor>/<service>/`):

\`\`\`bash
# Unit tests (mocha, nock-mocked, no credentials needed)
zbb test --slot local

# E2E in-process against the real API (needs credentials)
zbb testDirect --slot local

# E2E in container (validates packaging)
zbb testDocker --slot local
\`\`\`

### Test Environment Setup

E2E tests need credentials staged in the local zbb slot — never in `.env` files or committed code:

\`\`\`bash
zbb secret create SERVICE_EMAIL \\
  --module @zerobias-org/module-<vendor>-<service> --slot local
zbb secret create SERVICE_PASSWORD \\
  --module @zerobias-org/module-<vendor>-<service> --slot local
\`\`\`
...
## Support
For issues, contact <contact-email>
```
