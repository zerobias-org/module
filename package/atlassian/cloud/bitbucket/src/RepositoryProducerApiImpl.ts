...
import { RepositoryProducerApi, RepositoryApi } from '../generated/api/RepositoryApi';
...
export class RepositoryProducerApiImpl implements RepositoryProducerApi {
  ...
  async list(
    results: PagedResults<Repository>,
    workspace: string,
    role?: RepositoryApi.RoleEnumDef,
    q?: string,
    orderBy?: string
  ): Promise<void> {
    ...
    results.items = response.data.values.map(toRepository);
