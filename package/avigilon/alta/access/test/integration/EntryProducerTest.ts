  ...
  describe('Entries List Operations', () => {
    it('should retrieve entries list with full details', async function () {
      ...
      if (entries.length > 0) {
        ...
        expect(entry.id).to.be.greaterThan(0);
        ...
        if (entry.zone) {
          ...
          if (entry.zone.site) {
            ...
            expect(entry.zone.site.id).to.be.a('number');
        ...
        if (entry.entryState) {
          ...
          expect(entry.entryState.id).to.be.a('number');
        ...
        if (entry.schedule) {
          ...
          expect(entry.schedule.id).to.be.a('number');
        ...
        if (entry.cameras) {
          ...
          entry.cameras.forEach(camera => {
            ...
            expect(camera.id).to.be.a('number');
    ...
    it('should flatten response data array and drop metadata', async function () {
      ...
      // Verify we get a flat array, not wrapped in {data: [...], meta: {...}, totalCount: ...}
      expect(entries).to.be.an('array');
      
      // Verify there are no metadata properties on the array itself
      expect(entries).to.not.have.property('data');
      expect(entries).to.not.have.property('meta');
      expect(entries).to.not.have.property('totalCount');
      expect(entries).to.not.have.property('filteredCount');
    });
  });
});